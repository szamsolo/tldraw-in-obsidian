...
export default function deepLinkListener(plugin: TldrawPlugin, tFile: TFile, embed: MarkdownEmbed): TLDeepLinkOptions {
    const getInternalLinkToken = (widget: InternalEmbedWidget | undefined) => {
        const internalLinkToken = (() => {
            ...
            return !token || token.type !== 'internal-link'
                ? undefined
                : token
                ;
        })();
    ...
    return {
        onChange(url, editor) {
            ...
            const markdownLinkText = updateEmbedLinkText(internalLinkToken ?? {
